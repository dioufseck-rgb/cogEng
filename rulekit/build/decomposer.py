...
# ---------------------------------------------------------------------------
# LLM caller and JSON parsing (re-exports from build.llm)
# ---------------------------------------------------------------------------
# These utilities were extracted into rulekit/build/llm.py in Phase 1 of
# the contract migration so they have a stable home that isn't entangled
# with the dataclass spec layer being retired. The names are re-exported
# here for backward compatibility with existing consumers. New code
# should import from rulekit.build.llm directly.

from rulekit.build.llm import (  # noqa: E402, F401
    LLMCaller,
...
def _to_decimal_constant(value, label_for_error: str = "") -> Decimal:
    """Re-export of rulekit.build.llm.to_decimal_constant.

    Preserves the underscore-prefixed legacy spelling so internal
    callers in this module and external consumers continue working.
    The canonical spelling is `to_decimal_constant` in
    rulekit.build.llm. See that module's docstring for behavior.
    """
    from rulekit.build.llm import to_decimal_constant
    return to_decimal_constant(value, label_for_error)
