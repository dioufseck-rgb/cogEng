...
from rulekit.decomposer import LLMCaller, build_from_spec, BuildSpec, DeterminationDeclaration
...
from baselines.direct_llm import run_direct_llm, load_frozen_prompt, _normalize_kleene_str
...
def build_phase(policy_config: dict, builds_dir: str, llm: LLMCaller,
    ...
    if n_attempted > 0 and n_errored == n_attempted:
        # Every attempted build failed — that's a hard failure
        print(f"\nALL {n_attempted} ATTEMPTED BUILDS FAILED. See error JSONs in {builds_dir}")
        raise SystemExit(2)
    if n_errored > 0:
        print(f"\nWARNING: {n_errored}/{n_attempted} builds failed (continuing with successful ones)")
    return all_metadata
